...
- 문서 버전: v0.4
- 작성일: 2026-09-03 (최종 갱신: 2026-09-06)
...
- 변경 이력:
  ...
  - v0.3 → v0.4 (2026-09-06 세션): MVP 완성 + 실사용 배포까지 진행. 아래 "16. v0.4 세션 변경 로그" 참고.
...
## 15. 다음 단계 제안 (v0.3 시점, 완료됨)

1. ~~위 오픈 이슈(상황 카드 우선순위, 설명 문구 검수) 확정~~
2. ~~사용자가 실사 이미지 수집 진행 (부족분은 Claude가 AI 생성으로 보완)~~
3. ~~메인 Claude Code 세션에서 MVP 기능 구현~~
4. `design-enhancer` 서브에이전트 대신 사용자 피드백 기반 반복 수정으로 대체 진행
5. 별도 `code-reviewer` 서브에이전트 없이 수동 확인 후 배포

---

## 16. v0.4 세션 변경 로그 (2026-09-06)

MVP 구현 이후, 사용자 피드백을 반영해 다듬고 실제 기기에 배포까지 완료한 세션 기록.

### 16.1 콘텐츠/판정 수정
- 헷갈리던 이모지 2종 교체: 따분하다 😐→😑, 부럽다 😏→🤤 (다른 29개 감정과 겹치지 않는 것으로 재선정)
- 상황보기 카드 순서 고정: 데이터상 A/B 순서와 무관하게 **"지금 배우는 감정" 카드가 항상 먼저, "상대방 감정"이 다음**에 나오도록 로직 수정
- 표정 따라하기(클로징) 카드 재구성: 상대방 감정을 빼고 **지금 배우는 감정의 실사 사진 → 이모티콘** 순서로만 표시
- 실사 사진 31개 전체 확보 완료 (마지막까지 비어있던 안타깝다·피곤하다·자랑스럽다까지 채움, `MISSING_PHOTOS` 플레이스홀더 로직 제거)
- 홈 화면 모드 카드의 "🔤(abc)" 아이콘을 "의미(뜻)" 텍스트 배지로 교체

### 16.2 연출/사운드 반복 수정
- 카드별 "다 해봤어요!" 노출 시 아래→위로 솟아 터지는 폭죽 연출 추가, 이후 강도(샷 수·파티클 수·상승높이) 상향 + "펑"+크래클 노이즈 사운드 추가
- 라운드 완료 연출에 폭죽/비눗방울(사운드 포함) 추가
- 라운드 완료 마스코트(펭귄) 시도 → 제자리 회전 → 화면 하단에서 콩콩 뛰어다니는 방식으로 변경 → **최종적으로 사용자가 마스코트 자체를 별로라고 판단해 완전히 제거** (뽀로로 등 실제 캐릭터 IP는 라이선스 문제로 사용 불가 확인)
- 오답 사운드: 처음엔 소리가 약해 "삐~" 부저음으로 강화 → 이후 "너무 자극적"이라는 피드백으로 사각파(square)→사인파(sine) 기반의 부드러운 톤으로 재조정

### 16.3 배포
- 로컬 file:// 실행은 iPad Safari에서 스크립트/스타일 로드가 막혀 동작하지 않는 것을 확인 (모바일 Safari의 file:// 보안 제약)
- 1차: PC에서 `python -m http.server`로 같은 Wi-Fi 내 임시 접속 확인
- 2차(확정): **GitHub Pages로 이전** — 저장소 `https://github.com/planatoz2032-source/hm-expression` (main 브랜치, public) 생성 후 배포
  - 서비스 주소: **https://planatoz2032-source.github.io/hm-expression/**
  - `index.html`에 `apple-mobile-web-app-capable` 등 메타 태그 추가 → iPad에서 "홈 화면에 추가" 시 주소창 없는 전체화면 앱처럼 실행되도록 함
  - PC 전원/Wi-Fi 상태와 무관하게 상시 접속 가능

### 16.4 확장 아이디어 (오픈 이슈)
- 사용자가 같은 컨셉(카드 기반 매칭 학습)으로 **시제 맞추기 / 방향 맞추기 / 접속사** 등 후속 미니게임을 기획 중이며, 이 페이지와 연동하고 싶어함.
- 연동 방식(단일 허브 사이트에 게임 여러 개를 서브폴더로 둘지, 게임마다 별도 저장소 + 허브 랜딩 페이지로 링크만 걸지)은 아직 미확정 — 다음 세션에서 구조 확정 필요.
